...
/**
 * Resolve a subtitle entry's downloadable URL (handles both YouTube/Vimeo
 * style `formats.vtt` and yt-dlp style `url`).
 */
function subtitleUrl(sub, fmt = 'vtt') {
  if (sub?.formats && sub.formats[fmt]) return sub.formats[fmt];
  if (sub?.formats?.vtt) return sub.formats.vtt;
  return sub?.url || null;
}

/**
 * True when the user asked for subtitles (the default). Both
 * `--no-subtitle` and `--no-subtitles` disable the behaviour.
 */
function subtitlesEnabled(options) {
  return options.subtitle !== false && options.subtitles !== false;
}
 ...
 */
function parseRequestedLangs(value) {
  if (!value) return DEFAULT_SUB_LANGS.slice();
  const trimmed = String(value).trim();
  if (!trimmed) return DEFAULT_SUB_LANGS.slice();
  if (trimmed.toLowerCase() === 'all') return 'all';
  return trimmed.split(',').map(s => s.trim().toLowerCase()).filter(Boolean);
}

/**
 * Normalize the per-language `videoInfo.subtitles` dictionary so that
 * for every language we keep at most one track, preferring a manually
 * authored track over an auto-generated (ASR / "kind: asr") one.
 */
function dedupeSubtitles(subs) {
  ...
  for (const [lang, sub] of Object.entries(subs)) {
    if (!sub) continue;
    if (!sub.isAutoGenerated) out[lang] = sub;
  }
  for (const [lang, sub] of Object.entries(subs)) {
    if (!sub) continue;
    if (sub.isAutoGenerated && !out[lang]) out[lang] = sub;
  }
  return out;
}

/**
 * Resolve the list of subtitle downloads for a given videoInfo and the
 * current CLI options. Returns an array of `{url, lang, name}` entries
 * that the downloader knows how to fetch and embed via ffmpeg.
 *
 * Rules:
 *   - Disabled entirely when `--no-subtitle` / `--no-subtitles` is passed.
 *   - `--sub-translate <lang>` takes precedence over everything else:
...
function resolveSubtitleDownloads(videoInfo, options) {
  ...
  const rawSubs = videoInfo?.subtitles;
  if (!rawSubs || Object.keys(rawSubs).length === 0) return [];

  const manual = {};
  const asr = {};
  for (const [lang, sub] of Object.entries(rawSubs)) {
    if (!sub) continue;
    if (sub.isAutoGenerated) asr[lang] = sub; else manual[lang] = sub;
  }
  const deduped = dedupeSubtitles(rawSubs);
  const translateLang = options.subTranslate || null;
  const requested = parseRequestedLangs(options.subLang);

  const picks = [];
  const seen = new Set();
  const addPick = (url, lang, name) => {
    if (!url || !lang || seen.has(lang)) return;
    seen.add(lang);
    picks.push({ url, lang, name });
  };

  // 1. Explicit auto-translate override (pre-existing behaviour).
  if (translateLang) {
    const firstTrack = Object.values(deduped)[0];
    if (firstTrack?.isTranslatable) {
      const base = subtitleUrl(firstTrack, 'vtt');
      if (base) addPick(base + '&tlang=' + translateLang, translateLang, `${translateLang} (auto-translated)`);
    }
    return picks;
  }

  // 2. `--sub-lang all` — every native track + translate-fill.
  if (requested === 'all') {
    for (const [lang, sub] of Object.entries(deduped)) {
      const u = subtitleUrl(sub, 'vtt');
      if (!u) continue;
      const label = sub.isAutoGenerated ? `${sub.name || lang} (auto-generated)` : (sub.name || lang);
      addPick(u, lang, label);
    }
    if (options.subTranslateMissing !== false) {
      const trLangs = Array.isArray(videoInfo?.translationLanguages) ? videoInfo.translationLanguages : [];
      const firstTranslatable = Object.values(deduped).find(s => s?.isTranslatable);
      const base = firstTranslatable ? subtitleUrl(firstTranslatable, 'vtt') : null;
      if (base && trLangs.length > 0) {
        for (const tl of trLangs) {
          const code = tl.code || tl.languageCode;
          if (!code || seen.has(code)) continue;
          addPick(base + '&tlang=' + code, code, `${tl.name || code} (auto-translated)`);
        }
      }
    }
    return picks;
  ...
  const isTranslationLanguageAvailable = (code) => {
  ...
  for (const code of requested) {
    ...
    if (m) {
      ...
      if (u) { addPick(u, m.matchedLang, m.sub.name || m.matchedLang); continue; }
    }
    // 3b. Auto-generated (ASR) match.
    const a = findTrackByLang(asr, code);
    if (a) {
      const u = subtitleUrl(a.sub, 'vtt');
      if (u) { addPick(u, a.matchedLang, `${a.sub.name || a.matchedLang} (auto-generated)`); continue; }
    }
    // 3c. Auto-translate fallback — only if the language is known to be
    //     translatable by YouTube. Needs cookies to succeed (handled
    //     transparently by the downloader).
    if (translateBase && isTranslationLanguageAvailable(code)) {
      addPick(translateBase + '&tlang=' + code, code, `${code} (auto-translated)`);
      continue;
    }
    // 3d. Nothing available — skip silently.
  }
  return picks;
}
    ...
    try {
      ...
      if (formatPair && formatPair._hlsVideoUrl) {
        ...
        if (subtitleDownloads.length > 0) {
          ...
          console.log(chalk.yellow(`\u26A0 Requested subtitle track not available`));
        }
      ...
      if (videoInfo._type === 'playlist' && videoInfo.entries && videoInfo.entries.length > 0) {
        ...
        for (let idx = 0; idx < entries.length; idx++) {
          ...
          try {
            ...
            // Resolve subtitles
            const subtitleDownloads = resolveSubtitleDownloads(entryInfo, options);
