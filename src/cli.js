 ...
 * authored track over an auto-generated (ASR / "kind: asr") one. This
 * mirrors the user-requested behaviour:
 *   - Download ALL detected subtitles (manual + auto-generated).
 *   - When both a manual and an auto-generated track exist for the SAME
 *     language, keep only the manual one.
 *   - Auto-generated-only languages (e.g. Hungarian ASR) are kept as-is.
 */
function dedupeSubtitles(subs) {
  if (!subs || typeof subs !== 'object') return {};
  const out = {};
  // First pass: manual tracks win.
  for (const [lang, sub] of Object.entries(subs)) {
    if (!sub) continue;
    if (!sub.isAutoGenerated) out[lang] = sub;
  }
  // Second pass: ASR tracks fill in languages with no manual track.
  for (const [lang, sub] of Object.entries(subs)) {
    if (!sub) continue;
 ...
 *
 * Rules (see also `docs/README.md`):
 *   - Disabled entirely when `--no-subtitle` / `--no-subtitles` is passed.
 *   - `--sub-translate <lang>` takes precedence over everything else.
 *   - `--sub-lang <code>` downloads that single track (falling back to a
 *     machine translation when the language is not natively available
 *     but the source track is translatable).
 *   - `--sub-lang all` or no `--sub-lang` (the default) downloads every
 *     detected track, with manual captions preferred over auto-generated
 *     ones on a per-language basis.
 */
function resolveSubtitleDownloads(videoInfo, options) {
  ...
  const subs = dedupeSubtitles(rawSubs);
  ...
  if (translateLang) {
    ...
    if (firstTrack?.isTranslatable) {
      ...
      if (base) picks.push({ url: base + '&tlang=' + translateLang, lang: translateLang, name: `${translateLang} (auto-translated)` });
    }
  } else if (!requestedLang || requestedLang === 'all') {
    for (const [lang, sub] of Object.entries(subs)) {
      const u = subtitleUrl(sub, 'vtt');
      if (!u) continue;
      const label = sub.isAutoGenerated ? `${sub.name || lang} (auto-generated)` : (sub.name || lang);
      picks.push({ url: u, lang, name: label });
    }
  } else if (subs[requestedLang]) {
    const sub = subs[requestedLang];
    const u = subtitleUrl(sub, 'vtt');
    if (u) {
      const label = sub.isAutoGenerated ? `${sub.name || requestedLang} (auto-generated)` : (sub.name || requestedLang);
      picks.push({ url: u, lang: requestedLang, name: label });
    }
  } else {
    // Requested lang not natively present — try translation from the first track.
    const firstTrack = Object.values(subs)[0];
    if (firstTrack?.isTranslatable) {
      const base = subtitleUrl(firstTrack, 'vtt');
      if (base) picks.push({ url: base + '&tlang=' + requestedLang, lang: requestedLang, name: `${requestedLang} (auto-translated)` });
    }
  }
  return picks;
}
...
program
  ...
  .option('--sub-lang <lang>', 'Subtitle language code to download (e.g., en, hu, de). Use "all" for every detected track (default)')
      ...
      try {
        ...
        if (options.listFormats) {
          ...
          if (videoInfo.subtitles && Object.keys(videoInfo.subtitles).length > 0) {
            console.log(chalk.cyan('\nAvailable subtitles (all manual + auto-generated tracks are included by default; use --no-subtitles to skip):'));
